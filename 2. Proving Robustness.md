...
## Formalising $\epsilon$-ball robustness in Vehicle

In principle, $\epsilon$-ball robustness as a verification property bears some similarity to the ACAS Xu example that we already covered. In particular, both impose constraints on the output regions of the neural networks, given some constraint on its input regions. The main difference between the specifications is that ACAS Xu did not have to refer to the dataset; $\epsilon$-ball robustness, however, is formulated relative to the samples present in the dataset.   