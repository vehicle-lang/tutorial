...
$$|\mathbf{x}|_{\infty} = max (\mathbf{x})$$ where $max (\mathbf{x})$ computes the maximum element of $\mathfbf{x}$. Used to measure a distance between two vectors, it will measure the maximum difference between any two given elements in the vectors (taken pairwise). 
