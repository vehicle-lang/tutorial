...
It now remains to define the property "classification of $f$ does not change much". The paper we cite summarises a few options for this definition, but let us use the _standard robustness_ and take $|f(\hat{\mathbf{x}}) -  f(\mathbf{x})| \leq \delta$, for some small $\delta$.
And thus we get the disired property. Given an $\hat{\mathbf{x}}$,

$\forall \mathbf{x}. |\hat{\mathbf{x}}-\mathbf{x}| \leq \epsilon  \longrightarrow |f(\hat{\mathbf{x}}) -  f(\mathbf{x})| \leq \delta $

for some chosen metric $| ... |$, such as Eucledian distance or $L_{\infty}$-norm.. 