...
where $max (\mathbf{x})$ computes the maximum element of $\mathbf{x}$. When used to measure a distance between two vectors $\mathbf{x}_1$ and $\mathbf{x}_2$, $|\mathbf{x}_1 - \mathbf{x}_2|_{\infty}$ will measure the maximum difference between any two given elements in the vectors (taken pairwise). 
