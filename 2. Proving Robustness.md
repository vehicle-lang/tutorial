...
summarises a few options for this definition. The simplest is the 
...
## Exercise: Standard Robustness

Define and Verify in Vehicle the propety of _Standard Robustness_. 
_"standard robustness"_ that requires that $|f(\hat{\mathbf{x}}) -  f(\mathbf{x})| \leq \delta$, for some small $\delta$.
