...
where $max (\mathbf{x})$ computes the maximum element of $\mathbf{x}$. Used to measure a distance between two vectors, |\mathbf{x_1} - \mathbf{x_2}|_{\infty} will measure the maximum difference between any two given elements in the vectors (taken pairwise). 
