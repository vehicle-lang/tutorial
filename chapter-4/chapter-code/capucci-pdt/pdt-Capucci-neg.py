"""As pdt-Capucci.py, but SUBTRACTING the constraint loss instead of adding it.

The only change to the objective is the sign:

    total = ALPHA * cross_entropy - (1 - ALPHA) * constraint_loss

The motivation is the quantifier defect found on 2026-08-27. Vehicle 0.27.1 compiles a
`forall` into a loss that reports the property as *better* satisfied when the input region
is widened, and when the adversarial search is given more effort -- the opposite of what a
worst-case quantifier requires. If the compiled loss is inverted with respect to the
property, then minimising it trains away from robustness, and negating the coefficient
should compensate.

Read the result with care either way. In this logic `trueElement` is -infinity, so
minimising the constraint term (the positive-sign script) drives it toward "more true" in
the logic's own terms, while maximising it -- what this script does -- drives it toward
`falseElement`, +infinity. So on the logic's stated semantics this script trains the
network to violate the property. It is worth running only because we have evidence that
those stated semantics do not match what the compiled loss actually measures. Whichever
sign produces a better verification result tells us something about the defect, not about
which objective is principled.

Both signs are unbounded: one term runs to -infinity, the other to +infinity. Neither is
a well-posed objective on its own, which is why the task loss carries weight ALPHA.

Outputs go to traces-neg/ and capucci-models-neg/ so this run cannot disturb the
positive-sign one.
...
TRACES = os.path.join(HERE, "traces-neg")
MODELS = os.path.join(HERE, "capucci-models-neg")
...
def export(epoch: int) -> str:
    ...
    dst = os.path.join(MODELS, f"capucci_neg_e{epoch:02d}.onnx")
    was_training = model.training
    model.eval()
    torch.onnx.export(
        model,
...
per_epoch.close()
log(f"done; {NUM_EPOCHS} snapshots in capucci-models-neg/, statistics in traces-neg/per_epoch.csv")
