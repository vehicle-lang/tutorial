...
\begin{align*}
\only<1-4>{\large{\translation (\coloured{\forall \x \in \Real^n} : ||\x - \hat{\x}|| \leq \epsilon \Rightarrow || f(\x) - f(\hat{\x})|| \leq \delta) }}\\
\only<2-4>{\translation ( ||\x - \hat{\x}|| \leq \epsilon\ \coloured{\Rightarrow}\ || f(\x) - f(\hat{\x})|| \leq \delta) }\\
\only<3-4>{\max({1 - \translation(||\x - \hat{\x}|| \coloured{\leq} \epsilon)}, || \translation(f(\x) - f(\hat{\x})|| \coloured{\leq} \delta))}\\
\only<4>{ \max(\max(\dfrac{\translation (||\x - \hat{\x}|| - \epsilon)}{\translation (||\x - \hat{\x}|| + \epsilon)},0), 1 - \max(\dfrac{\translation(|| f(\x) - f(\hat{\x})|| - \delta)}{\translation (|| f(\x) - f(\hat{\x})|| + \delta)},0) }
\end{align*}
%\begin{align*}
...
\begin{frame}
 ...
 \begin{thebibliography}{99}
   \beamertemplatearticlebibitems
    \bibitem{6}{Marc Fischer, Mislav Balunovic, Dana Drachsler-Cohen, Timon Gehr, Ce Zhang, and
...
\end{thebibliography}}
 ...
 \begin{thebibliography}{99}
   \beamertemplatearticlebibitems
    \bibitem{6}{Emile van Krieken, Erman Acar, and Frank van Harmelen. Analyzing differentiable fuzzy
...
\end{thebibliography}}
 ...
 \begin{thebibliography}{99}
   \beamertemplatearticlebibitems
    \bibitem{6}{Peter Varnai and Dimos V. Dimarogonas. On robustness metrics for learning stl tasks.
