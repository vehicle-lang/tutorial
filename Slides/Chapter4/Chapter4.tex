...
\frametitle{Example - differentiable logic}
	We define a very simple differentiable logic on a toy propositional language $$a := a\ |p\ \leq\ p|\ a \land a\ |\ a \Rightarrow a$$ based on G\"{o}del fuzzy logic~[van Krieken 2022].
	\begin{align*}
		\translation (\val_1 \leq \val_2) &:=1-\max(\frac{\translation(\val_1) -\translation(\val_2)}{\translation(\val_1) + \translation(\val_2)}, 0)\\
		\translation (\val_1 \land \val_2)&:= \min(\translation(\val_1), \translation(\val_2))\\
		\translation (\val_1 \Rightarrow \val_2) &:= \max (1 - \translation(\val_1), (\translation))
	\end{align*}
\end{frame}
...
\pause
\begin{itemize}
     \item in Vehicle, we replace the above training objective with 
%\begin{equation}
$$\min_{\theta} [ \max_{\xs \in \mathbb{H}_{\mathcal{P}}} \lossfn_{\mathcal{S}}(\xs, \y)]$$
where $ \mathbb{H}_{\mathcal{P}}$ is a hyper-shape (usually a hyper-rectangle)
that corresponds to the pre-condition $\mathcal{P}$ of the property:
$\forall \xs. \mathcal{P} \Rightarrow \mathcal{S}$; and $\lossfn_{\mathcal{S}}$ is obtained by DL-translation of $\mathcal{S}$. 
...
\begin{itemize}
\item In AcasXu, $\mathbb{H}_{\mathcal{P}}$ is given by (the normalised) vector bounds given by the property:
...
\item  and $\lossfn_{\mathcal{S}} = \translation (not (minimalScore clearOfConflict x))$. 
...
\end{block}
\begin{itemize}
\item When we have $$\forall \xs. \mathcal{S}$$
instead of 
$\forall \xs. \mathcal{P} \Rightarrow \mathcal{S}$

\item $\mathbb{H}_{\mathcal{P}}$ is only constrained by normalisation boundaries, e.g. by $[0, 0 , \ldots , 0]$ and $[1, 1 , \ldots , 1]$ if we normalise values between $0$ and $1$.
...
\item $\mathbb{H}_{\mathcal{P}}$ is the $\epsilon$-cube around $\xt$

\item and, in case we use the G\"{o}del DL, we get the 
 $\lossfn_{\mathcal{S}} = \large{\translation ( || f(\x) - f(\hat{\x})|| \leq \delta) }=
1 - \max(\dfrac{\translation(|| f(\x) - f(\hat{\x})|| - \delta)}{\translation (|| f(\x) - f(\hat{\x})|| + \delta)},0) $
...
\begin{itemize}
\item the loss function $\lossfn_{\mathcal{S}}$ is automatically generated from the spec
\item the hyper-shape  $\mathbb{H}_{\mathcal{P}} $ is currently provided by the user;
\item PGD finds counter-examples within $\mathbb{H}_{\mathcal{P}} $
\item Native Python training is used, given the loss function $\lossfn_{\mathcal{S}}$.
\end{itemize}
