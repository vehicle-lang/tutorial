...
\frametitle{Example - differentiable logic}
	We define a very simple differentiable logic on a toy language $$p := p\ |a\ \leq\ a|\ p \land p\ |\ p \Rightarrow p$$ based on Product fuzzy logic~[van Krieken 2022].
	\begin{align*}
		\translation (\val_1 \leq \val_2) &:=1-\max(\frac{\val_1 -\val_2}{\val_1 + \val_2}, 0)\\
		\translation (p_1 \land p_2)&:= \translation(p_1) \times \translation(p_2)\\
		\translation (p_1 \Rightarrow p_2) &:=  1 - \translation(p_1) + \translation(p_1) \times \translation(p_2)
	\end{align*}
\end{frame}
...
\item and, in case we use the Product DL, we get \\
 $\lossfn_{\mathcal{S}(\xs)} = \large{\translation ( || f(\x) - f(\hat{\x})|| \leq \delta) }= 1 - \max(\dfrac{|| f(\x) - f(\hat{\x})|| - \delta}{|| f(\x) - f(\hat{\x})|| + \delta},0) $
