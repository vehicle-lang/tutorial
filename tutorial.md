...
Combined together, these four points make practical use of verification techniques inaccessible for majority of machine learning practicioners and even researchers, despite of the high demand for safety guarantees in complex intelligent systems. Imagine, for example, a designer of a chatbot who tries to prove that the chatbot does not offend or mislead a human user, ad suppose you were lucky enough to install one available neural network verifier.  For a start your neural network maybe too big to verify, but if you make it smaller, you still need to define formally what "offend or mislead" is, often in a very low-level language of SMT-Lib. If you have made through that hurdle, you may still find that your property fails for the model you have. So, you need to return to square one and train a different model, that does satisfy the property, your installed verifier will not be able to help with this.    
