...
Instead, we explain the main idea by means of example.
...
We now have all the necessary building blocks to define *Vehicle* approach to property-driven training,
...
where $\mathbb{H}_{\mathcal{P}(\mathbf{x})}$ is a hyper-shape 
that corresponds to the pre-condition $\mathcal{P}(\mathbf{x})$ of the property and $\mathcal{L}_{\mathcal{S}(\mathbf{x})}$ is obtained by DL-translation of $\mathcal{S}(\mathbf{x})$.
...
* $\mathbb{H}_{\mathcal{P}(\mathbf{x})}$ is the $\epsilon$-cube around $\hat{\mathbf{x}}$
