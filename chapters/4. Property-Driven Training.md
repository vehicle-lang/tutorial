...
*Gradient descent*  minimises loss $\mathcal{L}(\hat{\mathbf{x}}, \mathbb{y})$ between the predicted value $f_{\theta}(\hat{\mathbf{x}})$ and the true value $\mathbf{y}$, for each entry $(\hat{\mathbf{x}}, \mathbf{y})$ in $\mathcal{D}$. It thus solves the optimisation problem:
...
$$\min_{\theta} [ \max_{\mathbf{x} : \mathbf{x} - \hat{\mathbf{x}} \leq \epsilon} \lossfn(\mathbf{x}, \mathbf{y})]$$
 ...
 The inner maximisation is done by *``projected gradient descent"* (PGD), that ``projects" the gradient of $\mathcal{L}$ on $\xt$ in order to perturb it and get the worst $\mathbf{x}$.
