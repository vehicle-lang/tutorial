...
(with optimisation parameters $\theta$),
	a *loss function* 
...
The reader will find an excellent exposition of adversarial training in this tutorial: [@KM18].
...
$\forall \mathbf{x} \in \mathbb{B}(\hat{\mathbf{x}}, \epsilon). robust(f(\mathbf{x}))$. It turns out that adversarial training determines the definition of *robust* to be $ |f(\mathbf{x}) - f(\hat{\mathbf{x}})| \leq \delta$.

Moreover, we can map different kinds of training known in the machine learning literature to the following definitions of *robust*:
| Training style | Definition of *robust* |
| :---------------:| :---------------: | 
|Data Augmentation |	$ argmax\ [f(\mathbf{x})] = i$ |
...
where DL2 training is presented in [@FischerBDGZV19] and Lipschitz Continuity in [@Pauli0BKA21]. 
