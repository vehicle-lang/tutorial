...
![Data Manifold for D](images/SR-vs-CR-3.png)

However, data sets contain labels for data points, and not probabilities. We cannot achieve the effect we are looking for with data augmentation.
We have to modify our training algorithm instead [@GoodfellowSS14]. 

#### Loss Function

Given a data set $\mathcal{D} $,  a function  ${f_{\theta}: \mathbb{R}^n \rightarrow \mathbb{R}^m}$ (with optimisation parameters $\theta$),
	a \emph{loss function} $\mathcal{L}: \mathbb{R}^n \times \mathbb{R}^m \rightarrow \mathbb{R}$ computes a penalty proportional to the difference between the output of $f_{\theta}$ on a training input $\hat{\mathbb{x}}$ and a desired output $\mathbb{y}$.
...
*Gradient descent*  minimises loss $\mathcal{L}(\hat{\mathbb{x}}, \mathbb{y})$ between the predicted value $f_{\theta}(\hat{\mathbb{x}})$ and the true value $\mathbb{y}$, for each entry $(\hat{\mathbb{x}}, \mathbb{y})$ in $\mathcal{D}$. It thus solves the optimisation problem:
...
$$\min_{\theta} [ \max_{\mathbb{x} : \mathbb{x} - \hat{\mathbb{x}} \leq \epsilon} \lossfn(\mathbb{x}, \mathbb{y})]$$
 ...
 The inner maximisation is done by *``projected gradient descent"* (PGD), that ``projects" the gradient of $\mathcal{L}$ on $\xt$ in order to perturb it and get the worst $\mathbb{x}$.
