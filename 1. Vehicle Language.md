...
Some functions are pre-defined in **Vehicle**. For example, the above block uses multiplication `*`, division `/` and vector look up `!`.  We have also seen the use of a pre-defined "less or equal" predicate `<=` in the definition of the function `validInput` (note its `Bool` type).  
...
intruderDistantAndSlower x =
  ...
  x ! intruderSpeed      <= 60
```
Note the reasoning in terms of the ``problem space", i.e. the use of unnormalised input vectors. We have already encountered the vector lookup `!` before; but now we have a new predefined comparison function, `>=`, "greater or equal". The connective `and` is a usual Boolean connective (note the type of the function is `Bool`).  
...
To flag that this is the property we want Marabou to verify, we use the label `@property`, we have seen this notation when we flagged a neural network declaration. The final new bit of syntax we have not yet discussed is the quantifier `forall`.  
...
obtain the result -- the `property1` indeed holds for the given neural network, `acasXu_1_7.onnx`:
